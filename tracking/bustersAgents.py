...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            temp = max(ghost.values())
            ghostPosition = []
            for k, v in ghost.items():
                if v == temp:
                    ghostPosition.append(k)
            likelyPosition.append(ghostPosition)

        distance = []
        position = []
        for p in likelyPosition:
            temp = random.choice(p)
            position.append(temp)
            distance.append(self.distancer.getDistance(pacmanPosition, temp))

        shortestDistanceValue = min(distance)
        shortestDistanceIndex = distance.index(shortestDistanceValue)

        minimum = float('inf')
        finalAction = None
        for action in legal:
            temp = self.distancer.getDistance(position[shortestDistanceIndex], Actions.getSuccessor(pacmanPosition, action))
            if temp < minimum:
                minimum = temp
                finalAction = action
